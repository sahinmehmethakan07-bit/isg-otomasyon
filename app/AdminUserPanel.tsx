...
export function AdminUserPanel({ styles }: Props) {
  ...
  async function handleCreateUser(e: React.FormEvent) {
    ...
    // Admin'in mevcut email/şifresini al
    const adminEmail = auth.currentUser?.email;
    if (!adminEmail) {
      setStatus("❌ Admin oturumu bulunamadı.");
      setCreating(false);
      return;
    }

    // Admin'den şifresini iste (güvenli geri dönüş için)
    const adminPassword = window.prompt(
      "Güvenlik için admin şifrenizi girin (yeni kullanıcı oluşturduktan sonra oturumunuzu korumak için):"
    );
    if (!adminPassword) {
    ...
    try {
      // 1. Yeni kullanıcıyı oluştur (oturum değişir)
      ...
      // 2. Yeni kullanıcının Firestore profilini kaydet
      ...
      // 3. Yeni kullanıcıdan çıkış yap
      await signOut(auth);

      // 4. Admin olarak tekrar giriş yap
      await signInWithEmailAndPassword(auth, adminEmail, adminPassword);

      setStatus(`✅ ${newUser.displayName} (${ROLE_CONFIG[newUser.role].label}) başarıyla oluşturuldu. Admin oturumu korundu.`);
      setNewUser({ email: "", password: "", displayName: "", role: "doctor" });
      setShowForm(false);
      await loadUsers();
    } catch (err: any) {
      // Hata durumunda admin tekrar giriş yapmayı dene
      try {
        await signInWithEmailAndPassword(auth, adminEmail, adminPassword);
      } catch {}
